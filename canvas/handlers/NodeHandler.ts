...
class NodeHandler {
	 ...
	 */
	getNodePath = (target: NodeObject | any, nodes: NodeObject[] = [], direction = 'init') => {
		if (target) {
	...
	selectByPath = (path?: string[]) => {
		...
		const targetObjects = this.handler.objects.filter((object: any) => path.some(id => id === object.id));
		const nonTargetObjects = this.handler.objects.filter((object: any) => path.some(id => id !== object.id));
		nonTargetObjects.forEach((object: any) => {
			if (object.superType === 'link') {
				const { fromNode, toNode } = object as LinkObject;
				if (fromNode && toNode) {
					const fromIndex = targetObjects.findIndex((obj: any) => obj.id === fromNode.id);
					const toIndex = targetObjects.findIndex((obj: any) => obj.id === toNode.id);
	...
	deselect = () => {
		this.handler.objects.forEach((object: FabricObject) => {
			...
			if (object.superType === 'node') {
				const node = object as NodeObject | any;
				if (node.toPort) {
					node.toPort.set({
						opacity: 1,
					});
