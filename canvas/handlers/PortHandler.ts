...
class PortHandler {
		...
		if (toPort) {
			toPort.on('mouseover', () => {
				if (
					...
					if (toPort.links.some((link: any) => link.fromNode === this.handler.activeLine.fromNode)) {
						toPort.set({
							fill: 'red',
						});
	 ...
	 * @param {NodeObject} target
	 */
	setCoords = (target: NodeObject | any) => {
		if (target.toPort) {
			const toCoords = {
			...
			if (target.toPort.links.length) {
				target.toPort.links.forEach((link: any) => {
					const fromPort = link.fromNode.fromPort.filter((port: any) => port.id === link.fromPort.id)[0];
	 ...
	 * @param {NodeObject} target
	 */
	recreate = (target: NodeObject | any) => {
