...
export interface HandlerCallback {
	 ...
	 *
	 */
	onAdd?: (object: FabricObject | any) => void;
	/**
	 ...
	 *
	 */
	onSelect?: (target: FabricObject | any) => void;
	/**
	 ...
	 *
	 */
	onLoad?: (handler: Handler | any, canvas?: fabric.Canvas | any) => void;
}
...
class Handler implements HandlerOptions {
	public id?: string;
	public canvas?: FabricCanvas | any;
	public workarea: any;
	...
	public canvasOption?: CanvasOption | any = defaults.canvasOption;
	public gridOption?: GridOption | any = defaults.gridOption;
	...
	public imageHandler: any;
	public chartHandler: any;
	public elementHandler: any;
	public cropHandler: any;
	public animationHandler: any;
	public contextmenuHandler: any;
	public tooltipHandler: any;
	public zoomHandler: any;
	public workareaHandler: any;
	public interactionHandler: any;
	public transactionHandler: any;
	public gridHandler: any;
	public portHandler: any;
	public linkHandler: any;
	public nodeHandler: any;
	public alignmentHandler: any;
	public guidelineHandler: any;
	public eventHandler: any;
	public drawingHandler: any;
	public shortcutHandler: any;

	public objectMap: Record<string, FabricObject> = {};
	public objects: FabricObject[] | any;
	...
	constructor(options: HandlerOptions | any) {
	 ...
	 * @param {HandlerOptions} options
	 */
	public initOption = (options: HandlerOptions | any) => {
		this.id = options.id;
	 ...
	 */
	public getObjects = (): FabricObject[] | any => {
		const objects = this.canvas.getObjects().filter((obj: FabricObject | any) => {
			if (obj.id === 'workarea') {
				return false;
			} else if (obj.id === 'grid') {
				return false;
		...
		if (objects.length) {
			objects.forEach(obj => (this.objectMap[obj.id as any] = obj));
		} else {
			this.objectMap = {};
		}
		return objects;
	 ...
	 * @returns
	 */
	public setImage = (obj: FabricImage | any, source?: File | string) => {
		if (!source) {
			this.loadImage(obj, null as any);
			obj.set('file', null);
			obj.set('src', null);
			return;
		}
	 ...
	 */
	public centerObject = (obj: FabricObject | any, centered?: boolean) => {
		if (centered) {
	 ...
	 * @returns
	 */
	public add: any = (obj: any, centered = true, loaded = false) => {
		...
		if (this.objects.some((object: any) => object.type === 'gif')) {
	...
	public removeOriginById = (id: string) => {
		const object: any = this.findOriginByIdWithIndex(id);
		if (object.index > 0) {
		...
		if (clipboard.type === 'activeSelection') {
			if (clipboard.getObjects().some((obj: any) => obj.superType === 'node')) {
				...
				clipboard.getObjects().forEach((obj: any) => {
					...
					if (obj.fromPort.length) {
						obj.fromPort.forEach((port: PortObject | any) => {
							if (port.links.length) {
								port.links.forEach((link: LinkObject | any) => {
									const linkTarget = {
	 ...
	 * @param {FabricObject} obj
	 */
	public find = (obj: FabricObject | any) => this.findById(obj.id);
	...
	public findById = (id: string): FabricObject | null => {
		let findObject;
		const exist = this.objects.some((obj: any) => {
			if (obj.id === id) {
				findObject = obj;
				return true;
			}
	...
	public findOriginById = (id: string) => {
		let findObject: FabricObject | any;
		const exist = this.objects.some((obj: any) => {
			if (obj.id === id) {
				findObject = obj;
				return true;
	...
	public findOriginByIdWithIndex = (id: string) => {
		let findObject;
		let index = -1;
		const exist = this.objects.some((obj: any, i: any) => {
			if (obj.id === id) {
				findObject = obj;
				index = i;
				return true;
	 ...
	 * @param {boolean} [find]
	 */
	public select = (obj: FabricObject | any, find?: boolean) => {
		let findObject = obj;
		if (find) {
	 ...
	 * @param {number} height
	 */
	public originScaleToResize = (obj: FabricObject | any, width: number, height: number) => {
		if (obj.id === 'workarea') {
			this.setByPartial(obj, {
	...
	public scaleToResize = (width: number, height: number) => {
		const activeObject = this.canvas.getActiveObject() as FabricObject | any;
	...
	public importJSON = async (json: any, callback?: (canvas: FabricCanvas) => void) => {
		...
		if (typeof json === 'string') {
			...
			json = json.objects ? json.objects : json;
		}
		else{
			json = json.objects ? json.objects : json;
		}
		...
		json.forEach((obj: FabricObjectOption | any) => {
			if (obj.id === 'workarea') {
				return;
			}
	...
	public clear = (includeWorkarea = false) => {
		const ids = this.canvas.getObjects().reduce((prev: any, curr: any) => {
