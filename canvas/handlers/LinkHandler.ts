 ...
 */
class LinkHandler {
	private port: PortObject | any = null;
	handler: Handler;
	...
	init = (port: PortObject) => {
		...
		const fromNode = this.handler.objectMap[nodeId as any];
	 ...
	 * @returns
	 */
	create = (link: LinkOption | any, loaded = false) => {
		const fromNode = this.handler.objectMap[link.fromNodeId] as NodeObject | any;
		const fromPort = fromNode.fromPort.filter((port: any) => port.id === link.fromPortId || !port.id)[0];
	 ...
	 * @param {LinkObject} link
	 */
	removeFrom = (link: LinkObject | any) => {
		if (link.fromPort) {
			let index = -1;
	 ...
	 * @returns
	 */
	isDuplicate = (port: PortObject | any) => {
		const validate = port.links.some((link: any) => link.fromNode.id === this.handler.activeLine.fromNode.id);
